...
function _exists(req) {
    return function exists(bucket, key, callback) {
        ...
        return client.get(bucket, key, function (err, meta, _) {
...
function _get(req) {
    return function get(bucket, key, callback) {
        ...
        return client.get(bucket, key, function (err, meta, val) {
            if (err)
...
function _put(req) {
    return function put(bucket, key, value, meta, callback) {
        ...
        log.debug({bucket: bucket, key: key}, 'put entered');
        return client.put(bucket, key, value, function (err) {
            if (err)
                return callback(_error(req, err));
...
function _del(req) {
    return function del(bucket, key, callback) {
        var client = req.moray;
        var log = req.log;
        var opts = { requestId: req.req_id };

        log.debug({bucket: bucket, key: key}, 'del entered');
        return client.del(bucket, key, function (err) {
            if (err)
                return _error(req, err);

            log.debug({bucket: bucket, key: key, val: val}, 'del done');
...
function _search(req) {
    return function search(bucket, filter, callback) {
        ...
        return client.search(bucket, filter, function (err, results) {
            if (err)
                return callback(_error(req, err));
