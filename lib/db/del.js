...
// Remove reverse index to linked 'sdcaccountrole' or 'sdcaccountgroup'
// object classes when required:
function roleGroupReverseIndex(req, res, next) {
    if (req._entry.objectclass.indexOf('sdcaccountrole') === -1 &&
            req._entry.objectclass.indexOf('sdcaccountgroup') === -1) {
        return next();
    }
    ...
    var oc = (req._entry.objectclass.indexOf('sdcaccountrole') !== -1) ?
        'sdcaccountrole' : 'sdcaccountgroup';
    var target = (oc === 'sdcaccountrole') ?
                    'membergroup' : 'memberrole';
    var current = (oc === 'sdcaccountrole') ?
                    'memberrole' : 'membergroup';
