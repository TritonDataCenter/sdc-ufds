...
function filterForEach(filter, fn) {
    if (ldap.Filter.isFilter(filter)) {
        if (filter.filter) {
            filterForEach(filter.filter, fn);
        } else if (filter.filters) {
            filter.filters.forEach(function (child) {
                filterForEach(child, fn);
            });
        } else {
            fn(filter);
        }
...
function children(req, res, next) {
    ...
    if (req.isClogSearch) {
        ...
        filterForEach(req.filter, function (f) {
        ...
        // server-side sort parameters require the same alteration
        opts.sort.forEach(function (item) {
            if (item.attribute == 'changenumber') {
                item.attribute = '_id';
            }
        });
    } else {
