/*
 * Copyright (c) 2014, Joyent, Inc. All rights reserved.
 *
 * This file defines handlers for the UFDS server LDAP search operation.
 */

var ldap = require('ldapjs');
var util = require('util');


///--- Handlers

// Account sub-users login includes both, account UUID and login,
// need to return results w/o the UUID prefix
var UUID_RE = /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/;

function _subUser(entry) {
    if (entry.objectclass.indexOf('sdcaccountuser') === -1) {
        return (entry);
    }

    var login = entry.login[0];

    if (login.indexOf('/') === 36 && UUID_RE.test(login.substr(0, 36))) {
        login = login.substr(37);
    }

    entry.login = [login];
    return (entry);
}


function compareDN(dn1, dn2) {
    if (dn1.parentOf(dn2) || dn2.parentOf(dn1)) {
        return dn2.rdns.length - dn1.rdns.length;
    }

    if (dn1.equals(dn2)) {
        return 0;
    }

    return -1;
}

function isClogSearch(req) {
    return (req.config.changelog &&
            req.bucket == req.config.changelog.bucket);
}

function base(req, res, next) {
    // CAPI-430: The base-level object should be queried for both 'base' and
    // 'sub' scoped searches.  It should not be queried for 'one' scoped
    // searches or any search pertaining to the changelog.
    // (The changelog doesn't exist as a base object.)
    if (req.scope === 'one' || isClogSearch(req)) {
        return next();
    }

    return req.get(req.bucket, req.key, function (err, obj) {
        if (err) {
            return next(err);
        }

        if (req.filter.matches(obj.value)) {
            res.send({
                dn: req.dn,
                attributes: _subUser(obj.value)
            }, req.hidden);
        }

        return next();
    });
}


function children(req, res, next) {
    if (req.scope === 'base') {
        return next();
    }

    if (!isClogSearch(req)) {
        // Apply 'one' or 'sub' scope via filter
        req.filter = new ldap.AndFilter({
            filters: [req.filter]
        });
        if (req.scope === 'one') {
            req.filter.addFilter(
                new ldap.EqualityFilter({
                    attribute: '_parent',
                    value: req.dn.toString()
            }));
        } else if (req.scope === 'sub') {
            req.filter.addFilter(
                new ldap.SubstringFilter({
                    attribute: '_parent',
                    final: req.dn.toString()
            }));
        }
    }
    var filter = req.filter.toString();

    return req.search(req.bucket, filter, function (err, result) {
        if (err) {
            return next(err);
        }

        Object.keys(result).forEach(function (k) {
            var dist = compareDN(req.dn, ldap.parseDN(k));
            // HEAD-1278: moray already performs a full ldap evaluation
            // for us, so we really just want to check the DN. Also, we don't
            // have a good facility to deal with type coercion here.
            if ((req.scope === 'sub' && dist >= 0) ||
                    (req.scope === 'one' && dist === 1)) {
                res.send({
                    dn: k,
                    attributes: _subUser(result[k])
                }, req.hidden);
            }
        });

        return next();
    });
}


function done(req, res, next) {
    res.end();
    return next();
}


///--- Exports

module.exports = function searchChain() {
    return [base, children, done];
};
