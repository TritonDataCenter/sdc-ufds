...
/**
 * Tests if replication filter matches an entry
 */
function matches(replicator, parsedChanges) {
	if (replicator.replicationFilter.matches(parsedChanges))
		return true;
	else
		return false;
}
...
function getLocalEntry(replicator, entry, callback) {
	...
	function onSearch(err, res) {
		...
		function onErr(err) {
	        ...
	        log.fatal('Error searching for local entry');
			return callback(err);
		}
...
function modEntry(replicator, entry, callback) {
	...
	var targetdn = entry.object.targetdn;
	var changes = entry.entry.parsedChanges;
