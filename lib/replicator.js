...
Replicator.prototype.couldReplicate = function(targetdn, entry) {
	...
	if (targetdn.childOf(this.replicationDN)) {
		...
		// For deletes we can to take a look at the local entry first
...
function onSearch(self, err, res) {
	...
	function onSearchEnd(res) {
		...
		if (entries.length === 0) {
	    	updateChangenumber(self, self.changenumber + self.queueSize + 1,
	    	  function() {
				self.currPolling = false;
				log.trace('No new entries');
				log.trace('Updated changenumber to %s', self.changenumber);
		    });
		}
