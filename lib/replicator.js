...
function Replicator(options) {
	...
	/**
	 * Connect to the local UFDS client. When bound, connect to remote
	 *
	 * TODO: Initialization should be refactored on a chain method
	 */
	this.initLocal();
}
...
Replicator.prototype.initLocal = function() {
	var self = this;
	var log = self.log;
...
Replicator.prototype.initCheckpoint = function() {
	var self = this;
	...
	this.checkpoint.once('init', function (changenumber) {
...
function sort(a, b) {
	a = parseInt(a.changenumber, 10);
	b = parseInt(b.changenumber, 10);
