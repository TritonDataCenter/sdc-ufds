...
Replicator.prototype.couldReplicate = function(targetdn, entry) {
	...
	if (targetdn.childOf(this.replicationDN)) {
		if (changetype == 'modify' || changetype == 'delete')
			return true;
		else if (this.replicationFilter.matches(entry.parsedChanges))
			return true;
		else
			return false;
	} else {
...
function handleEntry(entry, callback) {
	...
	var changetype = entry.object.changetype;
  	...
  	switch (changetype) {
    	case 'add':
    		ops.add(self, entry, updateChangenumber);
    		break;
		case 'modify':
			ops.modify(self, entry, updateChangenumber);
			break;
		case 'delete':
			ops.del(self, entry, updateChangenumber);
			break;
    	default:
    ...
    function updateChangenumber(err) {
    	errors.ldapErrHandler(log, err);
	    ...
	    self.checkpoint.set(self.changenumber, function(err) {
			...
			self.log.info('changenumber %s processed successfully',
							self.changenumber)
