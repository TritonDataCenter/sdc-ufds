...
Replicator.prototype.initCheckpoint = function(next) {
	...
	var chkOps = {
		ufds: this.localUfds,
		url: this.remoteUfdsCfg.url,
		queries: this.remoteUfdsCfg.queries,
...
Replicator.prototype.parseReplicationQueries = function() {
	this.queries = [];

	assert.arrayOfString(this.remoteUfdsCfg.queries, 'remoteUfdsCfg.queries');

	for (var i = 0; i < this.remoteUfdsCfg.queries.length; i++) {
		var query = this.remoteUfdsCfg.queries[i];
		var url = this.remoteUfdsCfg.url + query;
		console.log(url);
	...
	function onSearchEnd(res) {
		...
		if (entries.length === 0) {
			...
			if (self.changenumber == self.searchnumber) {
				...
				if (!self.caughtUp) {
					self.caughtUp = true;
					self.emit('caughtup', self.changenumber);
				}
