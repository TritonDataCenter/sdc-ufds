...
Validator.prototype._add = function(entry, callback) {
  var i;
  var errors = [];
  ...
  for (i = 0; i < this._requiredKeys.length; i++) {
    ...
    if (entry.attributes[attrName].length > this.required[attrName]) {
  ...
  for (i = 0; i < attrs; i++) {
    ...
    if (entry.attributes[attrName].length > this.optional[attrName]) {
      errors.push(attrName +
                  ' can only have ' +
  ...
  if (typeof(this.add) == 'function')
    return this.add(entry, callback);
