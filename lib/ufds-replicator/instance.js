 ...
 */
Instance.prototype.localUfds = function() {
	...
	function onDrain() {
		...
		if (self.queueError === true) {
            ...
            updateChangenumber(self, self.searchnumber, function(err) {
                if (err) {
                    log.error(err, '[%d] updateChangenumber failed', self.id);
                } else {
...
function handleEntry(entry, callback) {
    ...
    function update(err) {
        if (err) {
            log.error(err, 'Replication operation failed', entry.object);
            self.queueError = true;
            return callback();
        }
...
function onSearch(self, err, res) {
		...
		if (entries.length === 0) {
			...
			// for us, so self.searchnumber > self.changenumber
                ...
                updateChangenumber(self, self.searchnumber, function(err) {
                    ...
                    if (err) {
                        log.error(err, '[%d] updateChangenumber failed',
                            self.id);
                    } else {
	...
	res.on('error', function(err) {
		self.currPolling = false;
		log.error(err, 'onError error produced');
	});
}
