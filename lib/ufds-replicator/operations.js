...
function getLocalEntry(replicator, entry, callback) {
    ...
    replicator.localUfds(function (ufds) {
        ...
        function onSearch(err, res) {
            ...
            function onErr(err1) {
                if (err1.code === ldap.LDAP_NO_SUCH_OBJECT) {
                    return callback();
                }

                log.fatal(err1, 'Error searching for local entry');
            ...
            function onSearchEnd(res1) {
                if (!localEntry) {
                    var err2 = new Error('No local entry but filter matched?');
 ...
 *   - ignore
 *
 * 3. The modified entry doesn't match the filter, but the unmodified one does.
 *   - ufds.del(targetdn)
 *
 ...
 *   - ufds.modify(targetdn, entry.parsedChanges)
 *
 * 5. The entry does not exist locally, but the modified one matches the filter
...
function modEntry(replicator, entry, callback) {
    ...
    replicator.localUfds(function (ufds) {
        ufds.modify(targetdn, changes, function (err, res) {
            if (err) {
                log.fatal(err, 'Could not modify entry %s', targetdn, changes);
            ...
            log.debug('Replicated entry modified %s', targetdn);
            return callback();
        });
    });
}
...
function delEntry(replicator, entry, callback) {
    ...
    replicator.localUfds(function (ufds) {
        ufds.del(targetdn, function (err, res) {
            if (err) {
                if (err.code &&
                    (err.code === ldap.LDAP_NOT_ALLOWED_ON_NON_LEAF)) {
                    log.error('Local entry %s has child entries and cannot ' +
                        'be deleted because of a ' +
                        'LDAP_NOT_ALLOWED_ON_NON_LEAF error, ignoring local ' +
                        'delete operation', targetdn);
                    return callback();
                } else {
                    return callback(err);
                }
