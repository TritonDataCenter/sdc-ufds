...
/*
 * Waits for the localUfds to be reconnected
 */
Replicator.prototype.ensureLocalUfds = function (callback) {
    var self = this;

    function ensure() {
        async.until(
            function () { return self.connecting === false; },
            function (cb) {
                setTimeout(cb, 100);
            },
            function () {
...
Replicator.prototype.stop = function () {
    ...
    async.mapSeries(self.remotes || [], function (remote, nextIter) {
        ...
        self.localUfds.unbind(function (err1) {
            if (err1) {
                self.log.fatal(err1, 'Error trying to unbind from Local');
            } else {
                self.emit('stopped');
            }
        });
